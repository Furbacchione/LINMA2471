...
\begin{example}
Let's apply proposittion \ref{prop1} with $f: x \rightarrow \sqrt{1+x^2}$
\begin{eqnarray*}
...
\begin{figure}[h]
\includegraphics[scale=0.6]{construction} 
\caption{In blue: f of $C_L^{1,1}$, in red: tangeant to the function, in green: the QUB}
\label{fig1}
\end{figure}


What can we say about the minimum of a function based only on its first derivative? We are tempted to use the Taylor series which approximate the function by its tangent.  Unfortunatly  we can't say anything from this about the minima. One better idea is to construct an upper bound approximation of our function using the Lipschitz constant then try to minimize this bound. We are thus sure that our fuction will stay below this bound, as shown in figure \ref{fig1}. 

\begin{lemma}
\textbf{Quadratic upper bound on $C_L^{1,1}$ function} \\
...
$f(N+1)-f(0) \leq - \dfrac{1}{L} \sum_{i=1}^{n}$ $\|\nabla f(x_{i})\|^{2}$\\
so we deduce \\
...
\begin{lemma}
Assume in addition that f is convex (meaning $f \in \mathbb{F}^{1,1}_{L}$ ) \footnote{f is convex and $f \in C_{L}^{1,1}$ }, then we have: 
$$ f(N)-f(x*) \leq \frac{L\|x_{0}-x*\|^{2}}{2N} $$ 
 (assuming x* is one of the minimum points).
\end{lemma}
 
\begin{example} Behavior of the gradient method
$$2x_{2}+y_{4}-2y_{2}$$
we start from (1,0)
